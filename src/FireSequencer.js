...
function saveGlobalData() {
  ...
  fs.writeFileSync(path.join(DATA_DIR, 'globalSave'), JSON.stringify(glob));
}

!function loadGlobalData() {
  let a = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'globalSave')));
...
function loadProject(IDuuid) {
  try {
    let projectFilename = path.join(PROJECTS_DIR, IDuuid.toString());
...
function deleteProject(IDuuid){
  ...
  try {
    fs.writeFileSync(path.join(DATA_DIR, 'saveFile'), JSON.stringify(projectsArray));
    fs.unlinkSync(path.join(PROJECTS_DIR, IDuuid.toString()))
    return true;
  } catch (err) {
    console.log(err)
    return err;
...
function saveProject(IDuuid, nameString) {
  ...
  if (found !== null) {
    ...
    try {
      fs.writeFileSync(path.join(DATA_DIR, 'saveFile'), JSON.stringify(tempObj));
    } catch (err) {
      console.log(err)
    }
  } else {
  ...
  try {
    fs.writeFileSync(path.join(PROJECTS_DIR, IDuuid.toString()), returnString);
  } catch (err) {
    console.log(err)
  }
  return IDuuid;
...
function getProjectsFromSaveFile() {
  return JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'saveFile')));
}
...
function writeTimingLogToFile() {
  fs.writeFileSync(path.join(DATA_DIR, 'timingLog.json'), JSON.stringify(timingLog));
}
