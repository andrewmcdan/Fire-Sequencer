  ...
  usbDetect.find(fireVID, firePID, function (err, devices) {
    if (!err) {
      if (devices.length != 0 && !FireSeq_isStarted) {
        ...
        console.log("Starting src/FireSequencer.js");
        let command = 'node';
        let parameters = [path.resolve(__dirname, 'FireSequencer.js')];
...
const seqLoop_parameters = [path.resolve(__dirname, 'seq_loop.js')];
