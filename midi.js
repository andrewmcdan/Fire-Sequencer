...
// track.forEach(function(track, index) {
//   track.patterns["id_0"].addEventByTime([144, 117, 0], 0);
// })
// track[1].patterns["id_0"].addEventByTime([144, 116, 0], 500);
// track[1].patterns["id_0"].addEventByTime([144, 116, 0], 1000);
// track[1].patterns["id_0"].addEventByTime([144, 116, 0], 1500);
// track[2].patterns["id_0"].addEventByTime([144, 115, 0], 0);
      ...
      case 144: // note-on event
        if (message[1] >= 54 && message[1] <= 117) { // grid button
          ...
          switch (seq.mode.current) {
            case 0: // step mode
              track = seq.track[seq.state.selectedTrackRange + ((btnIndex/16)&0xff)]; // determine which track is associated with row the button is on.
              if(track.patterns["id_"+seq.state.selectedPatternIndex].patIsStepBased){
                step = btnIndex%16; // step in the row
                track.patterns["id_"+seq.state.selectedPatternIndex].events["id_"+step].enabled = !track.patterns["id_"+seq.state.selectedPatternIndex].events["id_"+step].enabled;
                ipc.server.emit(seqLoop_ipcSocket, 'seq.trackVar', seq.track);
              }


              // set/unset note in track based on step position
              // send track data to seq_loop
              // ipc.server.emit(seqLoop_ipcSocket, 'seq.trackVar', seq.track);
              break;
            case 1: // note mode
              break;
